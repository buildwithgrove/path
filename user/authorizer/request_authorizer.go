...
// user.RequestAuthorizer is used to authorize service requests for specific gateway endpoints.
...
type (
	cache interface {
		GetGatewayEndpoint(ctx context.Context, endpointID user.EndpointID) (user.GatewayEndpoint, bool)
	}
	authorizer interface {
		authorizeRequest(ctx context.Context, reqDetails reqCtx.HTTPDetails, endpoint user.GatewayEndpoint) *failedAuth
	}
)
...
func (a *RequestAuthorizer) AuthorizeRequest(ctx context.Context, req *http.Request, endpointID user.EndpointID) gateway.HTTPResponse {

	reqDetails := reqCtx.GetHTTPDetailsFromCtx(ctx)

	endpoint, ok := a.cache.GetGatewayEndpoint(ctx, endpointID)
	if !ok {
		return &endpointNotFound
	}

	for _, auth := range a.authorizers {
		if resp := auth.authorizeRequest(ctx, reqDetails, endpoint); resp != nil {
			return resp
		}
	}
