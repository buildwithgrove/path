...
shannon_prepare_e2e_config: ## Setup Shannon E2E test config file from the example template
	@if [ ! -f ./e2e/.shannon.config.yaml ]; then \
		...
		echo "  - Copy/paste config into ./e2e/.shannon.config.yaml"; \
		echo ""; \
		echo "Then, for E2E tests:"; \
		echo "  make test_e2e_evm_shannon"; \
		echo ""; \
		echo "🧑‍💻 For local dev:"; \
		echo "  make shannon_populate_config OR make shannon_copy_e2e_config_to_local"; \
		echo "  make path_up"; \
		echo "################################################################"; \
	else \
		echo "################################################################"; \
...
.PHONY: shannon_copy_e2e_config_to_local
shannon_copy_e2e_config_to_local: ## Copy Shannon E2E config to local/path/ directory
	$(call check_config_exists,./e2e/.shannon.config.yaml,shannon_prepare_e2e_config)
