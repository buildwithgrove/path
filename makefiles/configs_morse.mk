...
morse_prepare_e2e_config: ## Setup Morse E2E test config file from the example template
	@if [ ! -f ./e2e/config/.morse.config.yaml ]; then \
		cp ./config/examples/config.morse_example.yaml ./e2e/config/.morse.config.yaml; \
		echo "################################################################"; \
		echo "Created ./e2e/config/.morse.config.yaml"; \
		echo ""; \
		echo "Next steps:"; \
		echo ""; \
		...
		echo "  2. Replace the contents of ./e2e/config/.morse.config.yaml with the config"; \
		echo ""; \
		...
		echo "################################################################"; \
		echo "Warning: ./e2e/config/.morse.config.yaml already exists"; \
		echo "To recreate the file, delete it first and run this command again"; \
		echo "	rm ./e2e/config/.morse.config.yaml"; \
...
morse_copy_e2e_load_test_config_to_local: ## Copy Morse E2E config to local/path/ directory
	...
	@echo "  From: ./e2e/config/.morse.config.yaml"
