// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.2
// 	protoc        v5.29.3
// source: path/protocol/shannon.proto

package protocol

import (
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	sync "sync"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

// ShannonObservations contains all the observations specific to relays served through the Shannon protocol.
type ShannonObservations struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// supplier_address contains the address of the supplier associated with the endpoint serving the request.
	SupplierAddress string `protobuf:"bytes,1,opt,name=supplier_address,json=supplierAddress,proto3" json:"supplier_address,omitempty"`
	unknownFields   protoimpl.UnknownFields
	sizeCache       protoimpl.SizeCache
}

func (x *ShannonObservations) Reset() {
	*x = ShannonObservations{}
	mi := &file_path_protocol_shannon_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ShannonObservations) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ShannonObservations) ProtoMessage() {}

func (x *ShannonObservations) ProtoReflect() protoreflect.Message {
	mi := &file_path_protocol_shannon_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ShannonObservations.ProtoReflect.Descriptor instead.
func (*ShannonObservations) Descriptor() ([]byte, []int) {
	return file_path_protocol_shannon_proto_rawDescGZIP(), []int{0}
}

func (x *ShannonObservations) GetSupplierAddress() string {
	if x != nil {
		return x.SupplierAddress
	}
	return ""
}

// ShannonObservationsList wraps ShannonObservations to enable embedding
// lists of Shannon observations in other protobufs.
type ShannonObservationsList struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Observations  []*ShannonObservations `protobuf:"bytes,1,rep,name=observations,proto3" json:"observations,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ShannonObservationsList) Reset() {
	*x = ShannonObservationsList{}
	mi := &file_path_protocol_shannon_proto_msgTypes[1]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ShannonObservationsList) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ShannonObservationsList) ProtoMessage() {}

func (x *ShannonObservationsList) ProtoReflect() protoreflect.Message {
	mi := &file_path_protocol_shannon_proto_msgTypes[1]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ShannonObservationsList.ProtoReflect.Descriptor instead.
func (*ShannonObservationsList) Descriptor() ([]byte, []int) {
	return file_path_protocol_shannon_proto_rawDescGZIP(), []int{1}
}

func (x *ShannonObservationsList) GetObservations() []*ShannonObservations {
	if x != nil {
		return x.Observations
	}
	return nil
}

var File_path_protocol_shannon_proto protoreflect.FileDescriptor

var file_path_protocol_shannon_proto_rawDesc = []byte{
	,
}

var (
	file_path_protocol_shannon_proto_rawDescOnce sync.Once
	file_path_protocol_shannon_proto_rawDescData = file_path_protocol_shannon_proto_rawDesc
)

func file_path_protocol_shannon_proto_rawDescGZIP() []byte {
	file_path_protocol_shannon_proto_rawDescOnce.Do(func() {
		file_path_protocol_shannon_proto_rawDescData = protoimpl.X.CompressGZIP(file_path_protocol_shannon_proto_rawDescData)
	})
	return file_path_protocol_shannon_proto_rawDescData
}

var file_path_protocol_shannon_proto_msgTypes = make([]protoimpl.MessageInfo, 2)
var file_path_protocol_shannon_proto_goTypes = []any{
	(*ShannonObservations)(nil),     // 0: path.protocol.ShannonObservations
	(*ShannonObservationsList)(nil), // 1: path.protocol.ShannonObservationsList
}
var file_path_protocol_shannon_proto_depIdxs = []int32{
	0, // 0: path.protocol.ShannonObservationsList.observations:type_name -> path.protocol.ShannonObservations
	1, // [1:1] is the sub-list for method output_type
	1, // [1:1] is the sub-list for method input_type
	1, // [1:1] is the sub-list for extension type_name
	1, // [1:1] is the sub-list for extension extendee
	0, // [0:1] is the sub-list for field type_name
}

func init() { file_path_protocol_shannon_proto_init() }
func file_path_protocol_shannon_proto_init() {
	if File_path_protocol_shannon_proto != nil {
		return
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: file_path_protocol_shannon_proto_rawDesc,
			NumEnums:      0,
			NumMessages:   2,
			NumExtensions: 0,
			NumServices:   0,
		},
		GoTypes:           file_path_protocol_shannon_proto_goTypes,
		DependencyIndexes: file_path_protocol_shannon_proto_depIdxs,
		MessageInfos:      file_path_protocol_shannon_proto_msgTypes,
	}.Build()
	File_path_protocol_shannon_proto = out.File
	file_path_protocol_shannon_proto_rawDesc = nil
	file_path_protocol_shannon_proto_goTypes = nil
	file_path_protocol_shannon_proto_depIdxs = nil
}
