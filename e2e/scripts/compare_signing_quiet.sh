...
if [[ ! -f "$PROJECT_ROOT/go.mod" ]] || ! grep -q "shannon-sdk" "$PROJECT_ROOT/go.mod" 2>/dev/null; then
    echo "Error: Invalid project or shannon-sdk not found"
    exit 1
fi
